...
import {
	parseCBDT,
	writeCBDT,
	writeCBDTComputeOffsets,
} from '../../src/sfnt/table_CBDT.js';
...
describe('CBDT table', () => {
	...
	it('should parse format 17 glyph records (small metrics + PNG)', () => {
		...
		cblcW.uint16(1);
		cblcW.uint16(17);
		cblcW.uint32(4); // imageDataOffset = 4
		cblcW.uint32(0);
		cblcW.uint32(14);
		cblcW.uint32(28); // sbitOffsets
	...
	it('should round-trip structured CBDT via coordinated write', () => {
		...
		expect(reparsedCBDT.bitmapData[0][0][0].imageData).toEqual([
			0x89, 0x50, 0x4e, 0x47,
		]);
		expect(reparsedCBDT.bitmapData[0][0][1].smallMetrics.height).toBe(12);
		expect(reparsedCBDT.bitmapData[0][0][1].imageData).toEqual([
			0x89, 0x50, 0x4e,
		]);
	});
	...
	it('should handle null entries for glyphs with no data', () => {
		...
		cblcW.uint32(0);
		cblcW.uint32(0);
		cblcW.uint32(14); // glyph 0: 0 bytes, glyph 1: 14 bytes
