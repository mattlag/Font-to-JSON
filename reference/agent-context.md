...
- **`src/`** — Library source. Entry: `src/main.js` exports `{ FontFlux, initWoff2 }`.
  ...
  - `src/glyph.js` / `src/kerning.js` / `src/json.js` / `src/svg_path.js` — Helpers
  - `src/sfnt/` — Shared table parsers (cmap, head, GPOS, COLR, etc.)
  - `src/otf/` — CFF/CFF2-specific (table_CFF.js, table_CFF2.js, cff_common.js, charstring_*.js)
...
**Key methods**: `FontFlux.open(buffer)`, `FontFlux.create({family, style, unitsPerEm, ...})`, `FontFlux.openAll(buffer)`, `.addGlyph()`, `.addKerning()`, `.export()`, `.validate()`, `.toJSON()`, `FontFlux.fromJSON()`.

**Instance properties** (live references): `.info`, `.glyphs`, `.kerning`, `.axes`, `.instances`, `.features`, `.tables`, `.glyphCount`, `.format`.
...
- **Tests**: 592+ passing across 72 files
...
- **FontFlux class**: Complete v2 API with glyph, kerning, axis, instance, feature, and hinting methods
...
- GSUB builder for simplified ligature/alternate authoring
